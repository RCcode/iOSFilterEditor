...
- (void)corner:(CornerView *)corner TouchesBegin:(UITouch *)touch{
    _isShowReseau = YES;
    [self setNeedsDisplay];
    _touchBeginPoint = [touch locationInView:corner];
}
...
- (void)corner:(CornerView *)corner TouchesMoved:(UITouch *)touch
{
    ...
    CGPoint locationInSuper = [self convertPoint:[touch locationInView:self.superview] fromView:self];
    CGPoint touchBeginInSuper = [self convertPoint:_touchBeginPoint toView:self.superview];
    
    //偏移量
    CGSize offset = CGSizeMake((corner.frame.size.width - _touchBeginPoint.x) - frame.origin.x,
                               (corner.frame.size.height - _touchBeginPoint.y) - frame.origin.y);
    ...
    //右上角
        ...
        frame.origin.y = locationInSuper.y - _touchBeginPoint.y + margin;
        frame.size = CGSizeMake(locationInSuper.x + offset.width - margin,
                                frame.size.height + (touchBeginInSuper.y - locationInSuper.y) - margin);
    //左上角
    }else if(corner == _leftUpBtn){
        frame.origin.y = locationInSuper.y - _touchBeginPoint.y + margin;
        frame.origin.x = locationInSuper.x - _touchBeginPoint.x + margin;
