...
//    NSLog(@"createAppsInfoTable");
    ...
    char *sql = "create table if not exists appsInfoTable(ID INTEGER PRIMARY KEY AUTOINCREMENT, appCate text,appComment int,appId int,appName text,bannerUrl text,downUrl text,iconUrl text,packageName text,price text,openUrl text int,appDesc text,state int)";
...
//        
//        //这里的数字1，2，3代表第几个问号，这里将两个值绑定到两个绑定变量
//        sqlite3_bind_int (statement, 1, (int)typeFaceObject.fontId);
...
//        sqlite3_bind_text(statement, 5, [typeFaceObject.previewUrl UTF8String], -1, SQLITE_TRANSIENT);
//        sqlite3_bind_int (statement, 6, (int)typeFaceObject.size);
//        sqlite3_bind_text(statement, 7, [typeFaceObject.downloadUrl UTF8String], -1, SQLITE_TRANSIENT);
//        sqlite3_bind_int (statement, 8, (int)typeFaceObject.version);
...
- (BOOL)insertAppInfo:(NSMutableArray *)appsInfo
{
	...
	if ([self openDB]) {
        ...
        for (ME_AppInfo *appInfo in appsInfo) {
            ...
            //这个 sql 语句特别之处在于 values 里面有个? 号。在sqlite3_prepare函数里，?号表示一个未定的值，它的值等下才插入。
            char *sql = "INSERT INTO appsInfoTable(appCate, appComment ,appId ,appName , bannerUrl, downUrl, iconUrl, packageName, price, openUrl ,appDesc ,state) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)";
            ...
            sqlite3_bind_text(statement, 11, [appInfo.appDesc UTF8String], -1, SQLITE_TRANSIENT);
            sqlite3_bind_int (statement, 12, appInfo.state);
            //执行插入语句
            success2 = sqlite3_step(statement);
            //释放statement
            sqlite3_finalize(statement);
...
- (NSMutableArray *)getAllAppsInfoData
{
	...
	if ([self openDB]) {
        ...
        char *sql = "SELECT appCate ,appComment ,appId ,appName ,bannerUrl ,downUrl ,iconUrl, packageName, price, openUrl ,appDesc ,state FROM appsInfoTable";
        ...
        if (sqlite3_prepare_v2(_database, sql, -1, &statement, NULL) != SQLITE_OK)
        {
            ...
            while (sqlite3_step(statement) == SQLITE_ROW) {
                ...
                char* appDesc  = (char*)sqlite3_column_text(statement, 10);
                appInfo.appDesc = [NSString stringWithUTF8String:appDesc];
                
                appInfo.state  = sqlite3_column_int(statement,11);
...
//            
//            while (sqlite3_step(statement) == SQLITE_ROW) {
//                FONT_fontTypeObject* sqlChat = [[FONT_fontTypeObject alloc] init] ;
//                
//                sqlChat.fontId  = sqlite3_column_int(statement,0);
//                
...
//                
//                char* purl  = (char*)sqlite3_column_text(statement, 4);
//                sqlChat.previewUrl = [NSString stringWithUTF8String:purl];
//                
//                sqlChat.size  = sqlite3_column_int(statement,5);
//                
//                char* url  = (char*)sqlite3_column_text(statement, 6);
//                sqlChat.downloadUrl = [NSString stringWithUTF8String:url];
//                
//                sqlChat.version  = sqlite3_column_int(statement,7);
//                
